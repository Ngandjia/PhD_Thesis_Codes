...
from sage.all import diff, var, expand, solve, factorial
...
def to_vector(covariant) :
    L = []
    n = degree_x(covariant)
    for i in range(n + 1) :
        derivative = derivative_x(derivative_y(covariant, i), n - i)
        j, k = factorial(i), factorial(n - i)
        if i == 0 :
            j = 1
        if n - i == 0 :
            k = 1
        L.append(derivative / (j * k))
    return (L)

def check_action(covariant, operator) :
    L = to_vector(covariant)
    L_acted = []
    for f in L :
        L_acted.append(operator(f))
    return (L_acted)
